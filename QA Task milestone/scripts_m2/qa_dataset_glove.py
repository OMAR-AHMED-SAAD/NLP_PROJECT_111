...
class QADatasetGlove(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        question_tokens, attention_mask_question, _ = self.encode(question, self.tokenizer)
        self.tokenizer.set_max_length(self.answer_max_length)
        answer_tokens, attention_mask_answer, _ = self.encode(answer, self.tokenizer)
        if self.include_context:
            self.tokenizer.set_max_length(self.context_max_length)
            context_tokens, attention_mask_context, context_offsets = self.encode(context, self.tokenizer, other_tokens_to_mask=["[SEP]", "[SOS]", "[EOS]"])
            self.tokenizer.set_max_length(self.context_max_length + self.question_max_length + 1) # +1 for [SEP]
            
            context_question_tokens, attention_mask_context_question = self.tokenizer.encode_two_texts(question, context, other_tokens_to_mask=["[SEP]", "[SOS]", "[EOS]"])
            # else:
        ...
        if self.include_context:
            ...
            returned_data["context_question"] = context_question_tokens.clone().detach()
